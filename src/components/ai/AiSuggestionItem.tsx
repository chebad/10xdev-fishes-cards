...
export default function AiSuggestionItem({ suggestion, onAccept, onReject, index }: AiSuggestionItemProps) {
  ...
  const hasChanges = useMemo(() => {
    return editState.question !== suggestion.suggestedQuestion ||
           editState.answer !== suggestion.suggestedAnswer;
  ...
  const getStatusBadge = () => {
    if (isAccepting) return <Badge variant="outline" className="text-blue-600">🔄 Zapisywanie...</Badge>;
    if (editState.isEditing) return <Badge variant="outline" className="text-amber-600">✏️ Edycja</Badge>;
    if (editState.hasChanges) return <Badge variant="outline" className="text-blue-600">📝 Zmodyfikowano</Badge>;
    return null;
  };
  ...
  return (
    <Card 
      className={`border-l-4 transition-all duration-200 hover:shadow-md focus-within:ring-2 focus-within:ring-blue-500 focus-within:ring-opacity-50 ${
        editState.isEditing ? 'border-l-amber-500 bg-amber-50/30' : 
        editState.hasChanges ? 'border-l-blue-500 bg-blue-50/30' : 
        'border-l-green-500'
      }`}
      role="article"
              ...
              {editState.isEditing ? (
                <div className="space-y-2">
                  ...
                  <div className="flex justify-between text-xs text-gray-500">
                    ...
                    {validationErrors.question && (
                      <span className="text-red-500">{validationErrors.question}</span>
                    )}
                  </div>
                </div>
              ) : (
                <div className="p-3 bg-gray-50 rounded border">
                  <p className="text-sm" id={`flashcard-${index}-description`}>
                    {editState.hasChanges ? editState.question : 
                     editState.question.length > 100 ? getPreviewText(editState.question) : editState.question}
                  </p>
              ...
              {editState.isEditing ? (
                <div className="space-y-2">
                  ...
                  <div className="flex justify-between text-xs text-gray-500">
                    ...
                    {validationErrors.answer && (
                      <span className="text-red-500">{validationErrors.answer}</span>
                    )}
                  </div>
                </div>
              ) : (
                <div className="p-3 bg-gray-50 rounded border">
                  <p className="text-sm whitespace-pre-wrap">
                    {editState.hasChanges ? editState.answer : 
                     editState.answer.length > 100 ? getPreviewText(editState.answer) : editState.answer}
                  </p>
            ...
            {editState.isEditing ? (
              <>
                ...
                <div className="text-xs text-gray-500 self-center mt-1 sm:mt-0">
                  💡 <kbd className="px-1 py-0.5 bg-gray-100 rounded text-xs">Ctrl+Enter</kbd> zapisuje, <kbd className="px-1 py-0.5 bg-gray-100 rounded text-xs">Escape</kbd> anuluje
                </div>
              </>
            ) : (
              <>
