...
describe("FlashcardService.createFlashcard", () => {
  ...
  describe("❌ Error Handling", () => {
    it("should throw error when Supabase returns database error", async () => {
      ...
      // 🚀 Act & Assert
      await expect(flashcardService.createFlashcard(command, userId)).rejects.toThrow(
        "Failed to create flashcard: duplicate key value violates unique constraint"
      );
    ...
    it("should throw error when no data returned despite no error", async () => {
      ...
      // 🚀 Act & Assert
      await expect(flashcardService.createFlashcard(command, userId)).rejects.toThrow(
        "Failed to create flashcard: no data returned."
      );
    });
    ...
    it("should handle network/connection errors", async () => {
      ...
      // 🚀 Act & Assert
      await expect(flashcardService.createFlashcard(command, userId)).rejects.toThrow("Network error");
    });
  });
  ...
  describe("🔄 Side Effects", () => {
    ...
    it("should handle session errors gracefully", async () => {
      ...
      await expect(flashcardService.createFlashcard(command, userId)).resolves.toEqual(
        expect.objectContaining({ id: "session-error-test" })
      );
    });
  });
  ...
  describe("🎭 Mock Verification", () => {
    it("should call Supabase methods in correct order", async () => {
      ...
      expect(mockSupabaseClient.from).toHaveBeenCalledWith("flashcards");
      expect(mockChain.insert).toHaveBeenCalledBefore(mockChain.select as ReturnType<typeof vi.fn>);
      expect(mockChain.select).toHaveBeenCalledBefore(mockChain.single as ReturnType<typeof vi.fn>);
    });
    ...
    it("should not call select().single() if insert fails", async () => {
      ...
      // 🚀 Act & Assert
      await expect(flashcardService.createFlashcard(command, userId)).rejects.toThrow("Insert failed");
